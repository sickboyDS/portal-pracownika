    ...
    //echo '<pre>' . print_r($_SESSION, TRUE) . '</pre>';
        ...
        try
        {
            ...
            if($conn->connect_errno!=0)
            {
                ...
                if($validate==true)
                {
                   if($conn->query("INSERT INTO users VALUES (NULL, '$login', '$pass_hash', '$email',  '$name', '$surname', '$adress', '$acn', '1', '2')"))
                   {
                    $_SESSION['info'] = "Konto zostało założone i musi zostać aktywowane przez administratora.";
                    header("Location: ../pages/registration.php");  
                   }
                   else{
